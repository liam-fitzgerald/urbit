...
export class Subscribers extends Component {
  ...
  render() {
    ...
    const readPerms = readPath ? this.props.permissions[readPath] : null;
    const writePath = this.props.notebook['writers-group-path'];
    const writePerms = writePath ? this.props.permissions[writePath] : null;
      ...
      writers = Array.from(withoutUs).map((who, i) => {
        ...
        if (readPath === writePath) {
          ...
          options = [
            {
              cls:
                'bg-transparent white-d tl pointer w-100 db hover-bg-gray4 hover-bg-gray1-d ph2 pv3',
    ...
    if (writers.length === 0) {
      writers = (
        <div className='f9'>There are no participants on this notebook.</div>
      );
    }
    ...
    if (readPath !== writePath) {
      if (this.props.notebook.subscribers) {
        ...
        subscribers = this.props.notebook.subscribers.map((who, i) => {
          const options = [
            {
              cls:
                'white-d tl pointer w-100 db hover-bg-gray4 hover-bg-gray1-d bg-transparent ph2 pv3',
            ...
            {
              cls:
                'tl red2 pointer w-100 db hover-bg-gray4 hover-bg-gray1-d bg-transparent ph2 pv3',
              txt: 'Ban',
              action: () => {
      ...
      if (subscribers.length === 0) {
        subscribers = (
          <div className='f9'>There are no subscribers to this notebook.</div>
        );
      }
    }

    const subsContainer =
      readPath === writePath ? null : (
    ...
    const addTags = [
      { description: 'Add Writer', tag: `writers-${this.props.book}` }
    ]

    const group = this.props.fullGroups[writePath];

    return (
      <Group resourcePath={writePath} group={group} tags={tags} addTags={addTags} api={this.props.api} />
    );
  }
}
